...
def apply_scaling(image: np.ndarray, scale_x: float, scale_y: float, interpolation_method) -> np.ndarray:
    """
    ...
    valid_pixels = interpolation_method(image, orig_x, orig_y).astype(np.uint8)
    scaled_image[valid_mask] = valid_pixels[valid_mask]

    return scaled_image

def apply_rotation(image: np.ndarray, angle: float, interpolation_method) -> np.ndarray:
    """
    ...
    valid_pixels = interpolation_method(image, x, y).astype(np.uint8)
...
# Interpolation methods available for the native implementation
INTERPOLATION_METHODS = {
    'nearest': {
        'native': nearest_neighbor,
        'opencv': cv.INTER_NEAREST
    },
    'bilinear': {
        'native': bilinear_interpolation,
        'opencv': cv.INTER_LINEAR
    },
    'bicubic': {
        'native': bicubic_interpolation,
        'opencv': cv.INTER_CUBIC
    },
    'lagrange': {
        'native': lagrange_interpolation,
        'opencv': cv.INTER_LANCZOS4
    }
}
...
def main() -> None:
    ...
    parser.add_argument("-cv2", "--opencv", action='store_false', help="Use the opencv geometric transformation")
    ...
    if args.command == "rotate":
        ...
        if args.opencv:
            output_image = apply_rotation(image=input_image, angle=angle, interpolation_method=INTERPOLATION_METHODS[args.method]['native'])
        else:
            output_image = apply_rotation_opencv(image=input_image, angle=angle, interpolation_method=INTERPOLATION_METHODS[args.method]['opencv'])
        ...
        if args.opencv:
            output_image = apply_scaling(image=input_image, scale_x=scale_x, scale_y=scale_y, interpolation_method=INTERPOLATION_METHODS[args.method]['native'])
        else:
            output_image = apply_scaling_opencv(image=input_image, scale_x=scale_x, scale_y=scale_y, interpolation_method=INTERPOLATION_METHODS[args.method]['opencv'])
    else:
        parser.print_help()
        sys.exit(2)
